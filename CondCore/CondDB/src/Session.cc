...
namespace cond {
  ...
  namespace persistency {
    ...
    Transaction::Transaction(SessionImpl& session) : m_session(&session) {}

    Transaction::Transaction(const Transaction& rhs) : m_session(rhs.m_session) {}
    ...
    Session::Session(const std::shared_ptr<SessionImpl>& sessionImpl)
        : m_session(sessionImpl), m_transaction(*m_session) {}

    Session::Session(const Session& rhs) : m_session(rhs.m_session), m_transaction(rhs.m_transaction) {}
    ...
    void Session::close() { m_session->close(); }

    Transaction& Session::transaction() { return m_transaction; }
    ...
    IOVEditor Session::createIovForPayload(const Hash& payloadHash,
      ...
      m_session->openDb();
      if (m_session->iovSchema().tagTable().select(tag))
        throwException("The specified tag \"" + tag + "\" already exist in the database.",
                       "Session::createIovForPayload");
      std::string payloadType("");
      if (!m_session->iovSchema().payloadTable().getType(payloadHash, payloadType))
        throwException("The specified payloadId \"" + payloadHash + "\" does not exist in the database.",
                       "Session::createIovForPayload");
      IOVEditor editor(m_session, tag, timeType, payloadType, synchronizationType);
      return editor;
    }
    ...
    GTEditor Session::createGlobalTag(const std::string& name) {
      ...
      if (m_session->gtSchema().gtTable().select(name))
        throwException("The specified Global Tag \"" + name + "\" already exist in the database.",
    ...
    cond::Hash Session::storePayloadData(const std::string& payloadObjectType,
                                         ...
                                         const boost::posix_time::ptime& creationTime) {
      m_session->openDb();
      return m_session->iovSchema().payloadTable().insertIfNew(
          payloadObjectType, payloadAndStreamerInfoData.first, payloadAndStreamerInfoData.second, creationTime);
    }
    ...
    coral::ISessionProxy& Session::coralSession() {
      if (!m_session->coralSession.get())
        throwException("The session is not active.", "Session::coralSession");
      return *m_session->coralSession;
    }

    coral::ISchema& Session::nominalSchema() { return coralSession().nominalSchema(); }

    TransactionScope::TransactionScope(Transaction& transaction) : m_transaction(transaction), m_status(true) {
    ...
    void TransactionScope::close() { m_status = true; }
